...
```{r}
library(readr)
Adults_Table_1 <- read_csv("~/Obesity+Prevalence+by+ZIP+Code+and+Gender,+Adults+and+Children/Adults-Table 1.csv")
View(Adults_Table_1)
```

```{r}
library(readr)
WI_Household_Income_by_Zip_1_ <- read_csv("~/WI Household Income by Zip (1).csv")
View(WI_Household_Income_by_Zip_1_)
```
...
```{r}
obesityrate <- Adults_Table_1
householdincome <- WI_Household_Income_by_Zip_1_
```
...
```{r}
ggplot(householdincome, aes(x = Income)) +
